...
class StoreWorkload extends WorkloadModuleBase {
     ...
     */
     _generateWorkload() {
        ...
        let workload = [];
        let roundIndexes = [9000,10000,12000,13000];
        // let roundIndexes = [0,200,400,600,1100,1600,2100];

        if(!isInit){
            isInit = true;
            var data = fs.readFileSync("./workload/scripts/setTaskTrainersData.json");
        ...
        for(let i= 0 ; i <  this.roundArguments.txnPerBatch ; i++) {
            ...
            let taskTrainers = myArray[this.txIndex+roundIndexes[this.roundArguments.num-1]].taskTrainers ; 
            let args = {
                contract :this.roundArguments.contract,
                verb: 'setTaskTrainers',
                args: [task,taskTrainers],
                readOnly: false,
