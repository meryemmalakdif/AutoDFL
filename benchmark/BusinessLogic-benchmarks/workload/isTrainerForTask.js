...
class StoreWorkload extends WorkloadModuleBase {
     ...
     */
     _generateWorkload() {
        ...
        let workload = [];
        let roundIndexes = [0,1];
        // let roundIndexes = [0,200,400,600,1100,1600,2100];

        if(!isInit){
            isInit = true;
            var data = fs.readFileSync("./workload/scripts/setTrainersData.json");
        ...
        for(let i= 0 ; i <  this.roundArguments.txnPerBatch ; i++) {
            this.txIndex++; 
            let adr = myArray[this.txIndex+roundIndexes[this.roundArguments.num-1]].trainer ; 
            console.log(adr)
            let args = {
                contract :this.roundArguments.contract,
                verb: 'historicalInteractions',
                args: ["0x01CD6F9962d2e6451143D20eEB93C0ECCAC241c9","0x25be2d1fd89c56f85cdf425e7068db172c9e82d1"],
                readOnly: false,
