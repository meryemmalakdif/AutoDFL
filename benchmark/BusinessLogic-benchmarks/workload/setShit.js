...
class StoreWorkload extends WorkloadModuleBase {
    ...
    _generateWorkload() {
        ...
        for(let i= 0; i < this.roundArguments.txnPerBatch; i++) {
            console.log("indice ",this.roundArguments.num-1)
            this.txIndex++;
            // let _modelCID = data[0][this.txIndex+roundIndexes[this.roundArguments.num-1]]._modelCID ; 
            // let _infoCID = data[0][this.txIndex+roundIndexes[this.roundArguments.num-1]]._infoCID ; 
            // let maxRounds = data[0][this.txIndex+roundIndexes[this.roundArguments.num-1]].maxRounds ; 
            // let requiredTrainers = data[0][this.txIndex+roundIndexes[this.roundArguments.num-1]].requiredTrainers ; 
            // let requiredAggregators = data[0][this.txIndex+roundIndexes[this.roundArguments.num-1]].requiredAggregators ; 
            let _modelCID = 2; 
            // let _infoCID = "QmUAXcaUnYuHjzyLhsjbYB1s2H29N8CRe8C5B5QADgoX8P" ; 
            // let maxRounds = 6 ; 
            // let requiredTrainers = 6 ; 
            // let requiredAggregators = 1 ; 
            let address = {address:'0x5839fbF17aAF78433386F364E80f749c27B0cd16'};
            let args = {
