...
class StoreWorkload extends WorkloadModuleBase {
     ...
     */
     _generateWorkload() {
        ...
        let currentNonce = 21;
//         const pendingTxCount = await this.sutAdapter.getTransactionCount(this.roundArguments.fromAddress, 'pending');
// console.log(`Pending transactions count: ${pendingTxCount}`);


        for(let i= 0; i < this.roundArguments.txnPerBatch; i++) {
            console.log(this.txIndex+roundIndexes[this.roundArguments.num-1])
            this.txIndex++;

            // let _modelCID = data[1][this.txIndex+roundIndexes[this.roundArguments.num-1]]._modelCID ; 
          
            let args = {
                contract :this.roundArguments.contract,
                verb: 'registerTaskPublisher',
                args: [],
                readOnly: false,
                nonce: currentNonce, // Use the incremented nonce
                gasPrice: '30000000000' // Set a suitable gas pr
            }
            currentNonce++;
