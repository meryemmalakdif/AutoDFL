...
contract BusinessLogic {
    ...
    function publishTask(uint256 taskId ,string memory _modelCID, string memory _infoCID, uint maxRounds, uint requiredTrainers) public {
        tasks.push(Task({
                taskId: taskId,
                modelCID: _modelCID,
                infoCID: _infoCID,
  ...
  function registerTrainer() public {
    ...
    function submitUpdate(Update memory modelUpdate,uint task,uint _round) public  {
        ...
        updatesCount[task][_round]++;
         ...
         // Wrapper function to measure gas for multiple `calculateObjectiveReputation` calls
    function multipleSubmitUpdate(
        Update[] memory taskIds, 
        uint[] memory taskPublishers, 
        uint256[] memory scores
    ) public  {
        require(
            taskIds.length == taskPublishers.length && 
            taskIds.length == scores.length, 
            "Input arrays must have the same length"
        );

        uint256[][] memory objectiveReps = new uint256[][](taskIds.length);
